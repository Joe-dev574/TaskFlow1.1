...
struct CustomTabBar: View {
    ...
    @Binding var activeTab: ItemCategory
        ...
        var body: some View {
            GeometryReader {
                let size = $0.size
                let allOffset = size.width - (10 * CGFloat(ItemCategory.allCases.count - 1))
                
                HStack(spacing: 5) {
                    HStack(spacing: activeTab == .dates ? -15 : 8) {
                        ForEach(ItemCategory.allCases.filter({ $0 != .dates }), id: \.rawValue) { tab in
        ...
        @ViewBuilder
        func ResizableTabButton(_ tab: ItemCategory) -> some View {
            HStack(spacing: 8) {
